...
• Real people, natural expressions
• Authentic interaction with the environment
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      // Handle content policy violations (safety system rejections)
      // Handle content policy violations (safety system rejections) - RETRY WITH SAFE PROMPT
      if (response.status === 400 && (
        ...
        console.warn(`[API] ⚠️ Content filter triggered. Retrying with simplified safe prompt using gpt-image-1...`)

        // Safe, minimal prompt
        const safePrompt = `Professional high quality photo of: ${topic}. 8k resolution, photorealistic.`
        ...
        if (safeResponse.ok) {
          ...
          const tempUrl = safeData.data[0].url
          const filename = slideId ? `slide-${slideId}-safe` : `slide-${Date.now()}-safe`
          const localPath = await saveImage(tempUrl, filename)
          return NextResponse.json({ imageUrl: localPath, generatedPrompt: safePrompt })
        } else {
          const safeError = await safeResponse.json().catch(() => ({}))
          console.error(`[API] ❌ Safe prompt retry failed: ${safeError.error?.message}`)

          // LEVEL 2 RETRY: Final Generic Fallback
          console.warn(`[API] ⚠️ Topic specific prompt failed. Trying generic professional background...`)
          const genericPrompt = `Abstract professional background image, high quality studio lighting, 8k resolution, neutral tones.`

          const genericResponse = await fetch("https://api.openai.com/v1/images/generations", {
            method: "POST",
            headers: {
              Authorization: `Bearer ${apiKey}`,
              "Content-Type": "application/json",
          ...
          if (genericResponse.ok) {
            ...
            const tempUrl = genericData.data[0].url
            const filename = slideId ? `slide-${slideId}-generic` : `slide-${Date.now()}-generic`
            const localPath = await saveImage(tempUrl, filename)
            return NextResponse.json({ imageUrl: localPath, generatedPrompt: genericPrompt })
          }

          // Return original error if ALL retries fail
          return NextResponse.json({
            error: "Image generation blocked by safety filters. Topic might be restricted."
          }, { status: 400 })
        }
      }
